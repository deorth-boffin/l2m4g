...
class TPapi:
    ...
    def __init__(self, url: str, passwd: str = None, encrypted: str = False, stok: str = None) -> None:
        self.url = url
        if not encrypted and passwd:
            self.passwd = TPapi.passwdEncryption(passwd)
        else:
            self.passwd = passwd
    ...
    def flushstok(self):
        data = {"method": "do", "login": {"password": self.passwd}}
        req = requests.post(url=self.url, json=data)
        try:
            self.stok = req.json()["stok"]
        except KeyError:
    ...
    def reconnectv4(self):
        ...
        self.apipost(data)
        data["network"]["change_wan_status"].update({"operate": "connect"})
        return self.apipost(data)
